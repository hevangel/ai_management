...
- Markdown rendered client-side by `marked` + `DOMPurify` (jsDelivr CDN). Routing uses the
  ...
  `RewriteRule . index.html [L]` in the tracked `.htaccess` (served from
  the repo; GitHub Pages ignores it, and it governs the horace.org mirror — the server copy should
  never diverge from the repo), and the repo's `404.html` shim
